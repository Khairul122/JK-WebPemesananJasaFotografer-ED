							...
							<div class="row">
								<div class="col-md-12">
									<div class="row">
										...
										<?php
										// Koneksi ke database (gantilah parameter koneksi sesuai dengan database Anda)
										...
										// Query untuk mengambil jumlah pemasukkan dari tabel transaksi
										$query = "SELECT SUM(pemasukkan) AS total_pemasukkan FROM transaksi";
										$result = $conn->query($query);

										// Periksa apakah query berhasil dijalankan
										if ($result) {
											$row = $result->fetch_assoc();
											$totalPemasukkan = $row['total_pemasukkan'];

											// Tampilkan jumlah pemasukkan pada panel HTML
											echo '
											<div class="col-md-4">
												<div class="panel panel-default">
													<div class="panel-body bk-warning text-light">
														<div class="stat-panel text-center">
															<div class="stat-panel-number h1">' . 'RP.' .'     ' . $totalPemasukkan . '</div>
															<div class="stat-panel-title text-uppercase">Pemasukkan</div>
														</div>
													</div>
												</div>
											</div>
										...
										// Query untuk mengambil jumlah pengeluaran dari tabel pengeluaran
										$query = "SELECT SUM(uang_keluar) AS total_pengeluaran FROM pengeluaran";
										$result = $conn->query($query);
										...
										if ($result) {
											...
											// Tampilkan jumlah pengeluaran pada panel HTML
											echo '
											<div class="col-md-4">
												<div class="panel panel-default">
													<div class="panel-body bk-warning text-light">
														<div class="stat-panel text-center">
															<div class="stat-panel-number h1">' . 'RP.' .'     '. $totalPengeluaran . '</div>
															<div class="stat-panel-title text-uppercase">Pengeluaran</div>
														</div>
													</div>
												</div>
											</div>
